...
class IssuedDocsDashboardView(BaseDashboardView):
    ...
    def get_buckets(self):
        ...
        buckets['Average nb of documentss received by day'] = map(lambda x: x['doc_count'] / 20.0, raw_buckets)
        buckets['% of documentss distributed late by DC'] = map(self.get_pc_late_distributed_docs, raw_buckets)
        buckets['% of documentss reviewed late by Leader'] = map(self.get_pc_late_leader_reviewed_docs, raw_buckets)
        buckets['Nb of documents with return code 0'] = map(self.get_nb_return_code_zero, raw_buckets)
        buckets['Nb of TR documentss in RC1'] = map(self.get_nb_tr_docs_in_rc(1), raw_buckets)
        buckets['Nb of TR documentss in RC2'] = map(self.get_nb_tr_docs_in_rc(2), raw_buckets)
        buckets['Nb of TR documentss in RC3'] = map(self.get_nb_tr_docs_in_rc(3), raw_buckets)
        buckets['Nb of vendor documentss in RC1'] = map(self.get_nb_vendor_docs_in_rc(1), raw_buckets)
        buckets['Nb of vendor documentss in RC2'] = map(self.get_nb_vendor_docs_in_rc(2), raw_buckets)
        buckets['Nb of vendor documentss in RC3'] = map(self.get_nb_vendor_docs_in_rc(3), raw_buckets)
...
class ReturnedDocsDashboardView(BaseDashboardView):
    ...
    def get_buckets(self):
        ...
        buckets['TOTAL'] = map(lambda x: x['doc_count'], raw_buckets)
        buckets['Avg nb of documentss returned by day'] = map(lambda x: x['doc_count'] / 20.0, raw_buckets)
        buckets['% of documentss returned late by GTG'] = map(self.get_pc_docs_returned_late_by_gtg, raw_buckets)
        buckets['% of TR documentss returned late by GTG'] = map(self.get_pc_tr_docs_returned_late_by_gtg, raw_buckets)
        ...
        buckets['% of Vendor documentss returned late by GTG'] = map(self.get_pc_vendor_docs_returned_late_by_gtg, raw_buckets)
